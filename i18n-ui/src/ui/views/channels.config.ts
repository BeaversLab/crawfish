...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout danger">${t("channelsDetail.schemaUnavailable")}</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout danger">${t("channelsDetail.configSchemaUnavailable")}</div>
    `;
  }
  ...
  return html`
    <div style="margin-top: 16px;">
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? t("nodesDetail.saving") : t("common.save")}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
