...
export function renderLogs(props: LogsProps) {
  ...
  const exportLabel = needle || levelFiltered ? t("logsDetail.exportFiltered") : t("logsDetail.exportVisible");
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("logsDetail.gatewayFileLogs")}</div>
        </div>
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="min-width: 220px;">
          ...
          <input
            ...
            placeholder="${t("logsDetail.searchLogs")}"
          />
        </label>
        <label class="field checkbox">
          <span>${t("logsDetail.autoFollow")}</span>
          <input
            type="checkbox"
      ...
      ${
        props.file
          ? html`<div class="muted" style="margin-top: 10px;">${t("logsDetail.file")}: ${props.file}</div>`
          : nothing
      }
      ${
        props.truncated
          ? html`
              <div class="callout" style="margin-top: 10px">${t("logsDetail.truncated")}</div>
            `
          : nothing
      }
      ${
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${
          filtered.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("logsDetail.noLogEntries")}</div>
              `
            : filtered.map(
                (entry) => html`
                <div class="log-row">
