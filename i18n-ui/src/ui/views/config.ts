...
function resolveSectionLabel(key: string) {
  const capKey = key.charAt(0).toUpperCase() + key.slice(1);
  return t(`config.section${capKey}`) !== `config.section${capKey}` ? t(`config.section${capKey}`) : key;
}
...
function resolveSectionMeta(
  ...
  if (meta) {
    return { ...meta, label: resolveSectionLabel(key) || meta.label };
  }
  return {
    label: resolveSectionLabel(key) || schema?.title || humanize(key),
          ...
          ${allSections.map(
            (section) => html`
              ...
              >
                ...
                >
                <span class="config-nav__label">${resolveSectionLabel(section.key)}</span>
              </button>
            `,
          )}
        </nav>
        ...
        <div class="config-actions">
                  ...
                  <span class="config-changes-badge"
                    >${
                      props.formMode === "raw"
                        ...
                        : t("config.viewChanges", {0: String(diff.length)})
                    }</span
                  >
                `
                : html`
          ...
          <div class="config-actions__right">
            ...
            <button
              ...
              @click=${props.onSave}
            >
              ${props.saving ? t("nodesDetail.saving") : t("common.save")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
            ...
            ? html`
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >${t("config.viewChanges", {0: String(diff.length)})}</span
                  >
                  <svg
