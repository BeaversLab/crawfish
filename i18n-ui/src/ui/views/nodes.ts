  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodesDetail.title")}</div>
          <div class="card-sub">${t("nodesDetail.subtitle")}</div>
        </div>
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("nodesDetail.loading") : t("nodesDetail.refresh")}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodesDetail.noNodes")}</div>
              `
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodesDetail.devices")}</div>
          <div class="card-sub">${t("nodesDetail.devicesSub")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("nodesDetail.loading") : t("nodesDetail.refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("nodesDetail.pending")}</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("nodesDetail.paired")}</div>
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">${t("nodesDetail.noPairedDevices")}</div>
              `
            : nothing
        }
      </div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim() ? `${t("nodesDetail.role")}: ${req.role}` : `${t("nodesDetail.role")}: -`;
  const repair = req.isRepair ? ` · ${t("nodesDetail.repair")}` : "";
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${role} · ${t("nodesDetail.requested")} ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${t("nodesDetail.approve")}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${t("nodesDetail.reject")}
          </button>
        </div>
      </div>
    </div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `${t("nodesDetail.roles")}: ${formatList(device.roles)}`;
  const scopes = `${t("nodesDetail.scopes")}: ${formatList(device.scopes)}`;
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t("nodesDetail.tokensNone")}</div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t("nodesDetail.tokens")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodesDetail.revoked") : t("nodesDetail.active");
  const scopes = `${t("nodesDetail.scopes")}: ${formatList(token.scopes)}`;
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodesDetail.bindings")}</div>
          <div class="card-sub">
            ${t("nodesDetail.bindingsSub")}
          </div>
        </div>
        <button
          class="btn"
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t("nodesDetail.switchToFormMode")}
              </div>
            `
          : nothing
      }
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodesDetail.loadConfigToEdit")}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("nodesDetail.loading") : t("nodesDetail.loadConfig")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t("nodesDetail.defaultBinding")}</div>
                  <div class="list-sub">${t("nodesDetail.defaultBindingDesc")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${t("nodesDetail.node")}</span>
                    <select
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${t("nodesDetail.anyNode")}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodesDetail.noNodesSystemRun")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("nodesDetail.noAgents")}</div>
                    `
...
function renderExecApprovals(state: ExecApprovalsState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodesDetail.execApprovals")}</div>
          <div class="card-sub">
            ${t("nodesDetail.execApprovalsSub")}
          </div>
        </div>
        <button
          class="btn"
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodesDetail.loadExecApprovals")}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("nodesDetail.loading") : t("nodesDetail.loadApprovals")}
            </button>
          </div>`
          : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesDetail.target")}</div>
          <div class="list-sub">
            ${t("nodesDetail.targetDesc")}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesDetail.host")}</span>
            <select
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>${t("nodesDetail.gateway")}</option>
              <option value="node" ?selected=${state.target === "node"}>${t("nodesDetail.node")}</option>
            </select>
          </label>
          ${
            state.target === "node"
              ? html`
                <label class="field">
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>${t("nodesDetail.selectNode")}</option>
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">${t("nodesDetail.noNodesExecApprovals")}</div>
            `
          : nothing
      }
    </div>
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesDetail.security")}</div>
          <div class="list-sub">
            ${isDefaults ? t("nodesDetail.securityDesc") : t("nodesDetail.defaultPrefix", { 0: defaults.security })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesDetail.mode")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodesDetail.useDefaultMode", { 0: defaults.security })}
                  </option>`
                  : nothing
              }
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesDetail.ask")}</div>
          <div class="list-sub">
            ${isDefaults ? t("nodesDetail.askDesc") : t("nodesDetail.defaultPrefix", { 0: defaults.ask })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodesDetail.useDefaultMode", { 0: defaults.ask })}
                  </option>`
                  : nothing
              }
              ${ASK_OPTIONS.map(
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesDetail.askFallback")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesDetail.askFallbackDesc")
                : t("nodesDetail.defaultPrefix", { 0: defaults.askFallback })
            }
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesDetail.fallback")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodesDetail.useDefaultMode", { 0: defaults.askFallback })}
                  </option>`
                  : nothing
              }
              ${SECURITY_OPTIONS.map(
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodesDetail.autoAllowSkills")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodesDetail.autoAllowSkillsDesc")
                : autoIsDefault
                  ? t("nodesDetail.autoAllowSkillsDefault", { 0: defaults.autoAllowSkills ? "on" : "off" })
                  : t("nodesDetail.autoAllowSkillsOverride", { 0: autoEffective ? "on" : "off" })
            }
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodesDetail.enabled")}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${t("nodesDetail.allowlist")}</div>
        <div class="card-sub">${t("nodesDetail.allowlistSub")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">${t("nodesDetail.noAllowlistEntries")}</div>
            `
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : t("nodesDetail.never");
  const lastCommand = entry.lastUsedCommand ? clampText(entry.lastUsedCommand, 120) : null;
  const lastPath = entry.lastResolvedPath ? clampText(entry.lastResolvedPath, 120) : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : t("nodesDetail.newPattern")}</div>
        <div class="list-sub">${t("nodesDetail.lastUsed", { 0: lastUsed })}</div>
        ${lastCommand ? html`<div class="list-sub mono">${lastCommand}</div>` : nothing}
        ${lastPath ? html`<div class="list-sub mono">${lastPath}</div>` : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodesDetail.pattern")}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t("nodesDetail.defaultAgent") : t("nodesDetail.agent")} ·
          ${
            bindingValue === "__default__"
              ? t("nodesDetail.usesDefault", { 0: state.defaultBinding ?? t("nodesDetail.anyNode") })
              : t("nodesDetail.override", { 0: agent.binding })
          }
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodesDetail.binding")}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : t("instances.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodesDetail.statusPaired") : t("nodesDetail.statusUnpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodesDetail.statusConnected") : t("nodesDetail.statusOffline")}
          </span>
