    ...
    <div class="agents-layout">
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("nav.navGroupAgent")}</div>
            <div class="card-sub">${agents.length} ${t("agentsDetail.configured")}.</div>
          </div>
      ...
      <section class="agents-main">
        ${
          !selectedAgent
            ? html`
                <div class="card">
                  <div class="card-title">${t("agentsDetail.selectAgent")}</div>
                  <div class="card-sub">${t("agentsDetail.selectAgentSub")}</div>
                </div>
              `
            : html`
              ${renderAgentHeader(
...
function renderAgentOverview(params: {
  ...
  return html`
    <section class="card">
      <div class="card-title">${t("nav.overview")}</div>
      <div class="card-sub">${t("overview.subtitle")}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.workspace")}</div>
          <div class="mono">${workspace}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.model")}</div>
          <div class="mono">${model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.identityName")}</div>
          <div>${identityName}</div>
          ${identityStatus ? html`<div class="agent-kv-sub muted">${identityStatus}</div>` : nothing}
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.default")}</div>
          <div>${isDefault ? t("common.yes") : t("common.no")}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.identityEmoji")}</div>
          <div>${identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsDetail.skillsFilter")}</div>
          <div>${skillFilter ? `${skillCount} selected` : t("agentsDetail.default")}</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">${t("agentsDetail.modelSelection")}</div>
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agentsDetail.primaryModel")}</span>
            <select
            ...
            >
              <option value="">
                ${defaultPrimary ? `${t("agentsDetail.inheritDefault")} (${defaultPrimary})` : t("agentsDetail.inheritDefault")}
              </option>
              ${buildModelOptions(configForm, effectivePrimary ?? undefined)}
            </select>
          </label>
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agentsDetail.fallbacks")}</span>
            <input
              .value=${fallbackText}
              ?disabled=${!configForm || configLoading || configSaving}
        ...
        <div class="row" style="justify-content: flex-end; gap: 8px;">
          ...
          <button
            ...
            @click=${onConfigSave}
          >
            ${configSaving ? t("nodesDetail.saving") : t("common.save")}
          </button>
        </div>
      </div>
    </section>
